...
$articles = getNewsList($category, $limit, $offset);
?>
<!DOCTYPE html>
<html lang="ru">
    <head>
        ...
        <title>Пресс-центр - Русский крест</title>
    ...
    <body>
        ...
        <header class="header">
            <div class="container">
                ...
                <a class="donate-btn  btn  btn--red" href="index.php#donation">
        ...
        <main>
            <section class="press press--page">
                <div class="container">
                    <div class="title-wrapper">
                        <h1 class="h4">Пресс-центр</h1>
                    </div>

                    <div class="news-filters">
                        <a href="news.php" class="filter-btn <?= !$category ? 'active' : '' ?>">Все</a>
                        <a href="news.php?category=news" class="filter-btn <?= $category === 'news' ? 'active' : '' ?>">Новости</a>
                        <a href="news.php?category=media" class="filter-btn <?= $category === 'media' ? 'active' : '' ?>">СМИ о Фонде</a>
                        <a href="news.php?category=events" class="filter-btn <?= $category === 'events' ? 'active' : '' ?>">События Фонда</a>
                    </div>

                    <ul class="press-list" id="news-list">
                        <?php if (empty($articles)): ?>
                            <li class="no-results">Новостей не найдено</li>
                        <?php else: ?>
                            <?php foreach ($articles as $index => $article): ?>
                                <li <?= $index === 0 && $offset === 0 ? 'class="press-list__item--featured"' : '' ?>>
                                    <a href="article.php?slug=<?= urlencode($article['slug']) ?>">
                                        <picture>
                                            <img src="<?= escapeHtml($article['image_url']) ?>" alt="<?= escapeHtml($article['title']) ?>">
                                        </picture>
                                        <div class="badge"><?= escapeHtml(getCategoryName($article['category'])) ?></div>
                                        <h6><?= escapeHtml($article['title']) ?></h6>
                                        <div class="row">
                                            <p><?= escapeHtml(formatDate($article['published_at'])) ?></p>
                                        </div>
                                    </a>
                                </li>
                            <?php endforeach; ?>
                    ...
                    <?php if (count($articles) >= $limit): ?>
                        <div class="load-more-wrapper" style="display: flex; justify-content: center; margin-top: 40px;">
                            <a href="news.php?<?= $category ? 'category=' . urlencode($category) . '&' : '' ?>offset=<?= $offset + $limit ?>" class="btn btn--light-blue">
